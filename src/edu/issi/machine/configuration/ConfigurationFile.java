...
public class ConfigurationFile extends File {
	 ...
	 */
	public ConfigurationFile(File file) throws InvalidAttributesException,
	...
	private void ensureValidity() throws InvalidAttributesException,
			FileNotFoundException {
		if (!this.exists()) {
			throw new FileNotFoundException(
					"Podany plik konfiguracyjny nie istnieje!");
		}
		if (!this.isFile() || !this.canRead()) {
			throw new InvalidAttributesException(
					"Podany plik jest niepoprawny lub niemo¿liwy do odczytu!");
		}
		if (!this.getName().endsWith(".json")) {
			throw new InvalidAttributesException(
					"Niepoprawny format pliku konfiguracyjnego!");
		}
	}
